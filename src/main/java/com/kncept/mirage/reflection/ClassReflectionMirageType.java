...
public class ClassReflectionMirageType implements MirageType {
	...
	public List<MirageType> getGenerics() {
		...
		if (genericInfo == null)
			System.out.println("genericInfo is null");
		else
			System.out.println("genericInfo is " + genericInfo.getClass().getName());
			...
			for (Type type : parameterized.getActualTypeArguments()) {
				if (type instanceof WildcardType) {
					...
					if (lowerBound == 0 && upperBound == 1) {
						...
						System.out.println(
								"wildcardType.getLowerBounds().length " + wildcardType.getLowerBounds().length);
						System.out.println(
								"wildcardType.getUpperBounds().length " + wildcardType.getUpperBounds().length);

						System.out.println("wildcardType.getUpperBounds()[0].getClass().getName() "
								+ wildcardType.getUpperBounds()[0].getClass().getName());
						System.out.println("((Class)wildcardType.getUpperBounds()[0]).getName() "
								+ ((Class) wildcardType.getUpperBounds()[0]).getName());
